...
async function saveImageToMoodboard(data: {
	...
	try {
		// Create 7.superdesign/moodboard directory if it doesn't exist
		const moodboardDir = vscode.Uri.joinPath(workspaceFolder.uri, '7.superdesign', 'moodboard');
		...
		try {
			...
			Logger.info('Created 7.superdesign/moodboard directory');
		}
...
async function getCssFileContent(filePath: string, sidebarProvider: ChatSidebarProvider) {
	try {
		...
		// If path doesn't start with 7.superdesign, add it
		if (!filePath.startsWith('7.superdesign/') && filePath.startsWith('design_iterations/')) {
			resolvedPath = `7.superdesign/${filePath}`;
		}
...
async function initializeSuperdesignProject() {
	...
	const superdesignFolder = vscode.Uri.joinPath(workspaceRoot, '7.superdesign');
...
- When creating design file:
  ...
  - You ALWAYS output design files in '7.superdesign/design_iterations' folder as {design_name}_{n}.html (Where n needs to be unique like table_1.html, table_2.html, etc.) or svg file
...
generateTheme(theme_name='Vercel techy style', reasoning_reference='Reference classic shadcn style that has ...', cssFilePath='7.superdesign/design_iterations/theme_1.css', cssSheet=':root {
  --background: oklch(1.0000 0 0);
...
write(file_path='7.superdesign/design_iterations/chat_ui.css', content='...')
write(file_path='7.superdesign/design_iterations/chat_ui.html', content='...')
...
3. You MUST use 7.superdesign/design_iterations folder to save the design files, do NOT save to other folders
...
@media (prefers-reduced-motion: reduce) {
    * {
	...
	try {
		// Create 7.superdesign/design_iterations directory
		...
		vscode.window.showInformationMessage('✅ Superdesign project initialized successfully! Created 7.superdesign folder and design rules for Cursor, Claude, and Windsurf.');
...
class SuperdesignCanvasPanel {
	...
	private _setupFileWatcher() {
		...
		// Watch for changes in 7.superdesign/design_iterations/*.html, *.svg, and *.css
		const pattern = new vscode.RelativePattern(
			workspaceFolder, 
			'7.superdesign/design_iterations/**/*.{html,svg,css}'
		);
	...
	private async _loadDesignFiles() {
		...
		try {
			const designFolder = vscode.Uri.joinPath(workspaceFolder.uri, '7.superdesign', 'design_iterations');
			...
			try {
				...
				try {
					...
					Logger.info('Created 7.superdesign/design_iterations directory');
				} catch (createError) {
					this._panel.webview.postMessage({
						command: 'error',

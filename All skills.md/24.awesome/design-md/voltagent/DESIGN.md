...
Typography leans on the system font stack for headings — achieving maximum rendering speed and native-feeling authority — while PingFang SC carries the body and UI text with geometric precision. Code blocks use SFMono-Regular, the same font developers see in their terminals, reinforcing the tool's credibility at every scroll.
...
- Dual-typography system: system-ui for authoritative headings, PingFang SC for precise UI/body text, SFMono for code credibility
...
- **Secondary (Body/UI)**: `PingFang SC`, with fallbacks inheriting from system-ui stack. OpenType features: `"calt", "rlig"` (contextual alternates and required ligatures)
...
| Sub-heading Light | system-ui / PingFang SC | 24px (1.50rem) | 300–400 | 1.33 | -0.6px | Light weight variant for softer hierarchy |
| Overline | system-ui | 20px (1.25rem) | 600 | 1.40 | 0.5px | Uppercase transform, positive letter-spacing |
| Feature Title | PingFang SC | 20px (1.25rem) | 500–600 | 1.40 | normal | Card headings, feature names |
| Overline Small | PingFang SC | 18px (1.13rem) | 600 | 1.56 | 0.45px | Uppercase section labels |
| Body / Button | PingFang SC | 16px (1.00rem) | 400–600 | 1.50–1.65 | normal | Standard text, nav links, buttons |
| Nav Link | PingFang SC | 14.45px (0.90rem) | 500 | 1.65 | normal | Navigation-specific sizing |
| Caption / Label | PingFang SC | 14px (0.88rem) | 400–600 | 1.43–1.65 | normal | Descriptions, metadata, badge text |
| Tag / Overline Tiny | system-ui | 14px (0.88rem) | 600 | 1.43 | 2.52px | Widest letter-spacing — reserved for uppercase tags |
| Micro | PingFang SC | 12px (0.75rem) | 400–500 | 1.33 | normal | Smallest sans-serif text |
...
- **OpenType by default**: Both system-ui and PingFang SC enable `"calt"` and `"rlig"` features, ensuring contextual character adjustments and ligature rendering throughout.
...
- Link text: Snow White (`#f2f2f2`) at 14–16px PingFang SC, weight 500
...
- Pair system-ui for headings with PingFang SC for body text — the speed/authority of native fonts combined with the precision of a geometric sans
...
- "Create a feature card on Carbon Surface (#101010) with a 1px solid Warm Charcoal (#3d3a39) border, comfortably rounded corners (8px). Use Snow White (#f2f2f2) for the title in system-ui at 24px weight 700, and Warm Parchment (#b8b3b0) for the description in PingFang SC at 16px. Add a subtle Warm Ambient shadow (rgba(92, 88, 85, 0.2) 0px 0px 15px)."
- "Design a ghost button with transparent background, Snow White (#f2f2f2) text in PingFang SC at 16px, a 1px solid Warm Charcoal (#3d3a39) border, and subtly rounded corners (6px). Padding: 12px vertical, 16px horizontal. On hover, background shifts to rgba(0, 0, 0, 0.2)."
...
- "Design a navigation bar on Abyss Black (#050507) with the VoltAgent logo (bolt icon with animated green glow) on the left, nav links in PingFang SC at 14px weight 500 in Snow White, and a green CTA button (Carbon Surface bg, VoltAgent Mint text) on the right. Add a 1px solid Warm Charcoal bottom border."
...
6. Always specify which font — system-ui for headings, PingFang SC for body/UI, SFMono-Regular for code

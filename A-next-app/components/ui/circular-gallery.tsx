...
class App {
  ...
  autoScrollSpeed: number = 0.035;
  autoScrollPauseTimer: number = 0;
  autoScrollPauseDuration: number = 60;
  lastSnappedIndex: number = -1;
  mouseX: number = 0;
  clickedMediaIndex: number = -1;
  clickPauseTimer: number = 0;
  start: number = 0;
  screen!: { width: number; height: number };
  viewport!: { width: number; height: number };
  raf!: number;
  ...
  onTouchUp(e?: MouseEvent | TouchEvent) {
    if (this.isDown) {
      const endX = e && "clientX" in e ? e.clientX : this.start;
      const moved = Math.abs(endX - this.start);
      if (moved < 5 && this.medias) {
        const rect = this.container.getBoundingClientRect();
        const clickNorm = ((endX - rect.left) / rect.width) * 2 - 1;
        let closest = -1;
        let minDist = Infinity;
        this.medias.forEach((media, i) => {
          const normalizedX = media.plane.position.x / (this.viewport.width / 2);
          const dist = Math.abs(normalizedX - clickNorm);
          if (dist < minDist) {
            minDist = dist;
            closest = i;
          }
        ...
        if (closest >= 0) {
          this.clickedMediaIndex = closest;
          this.clickPauseTimer = 120;
        }
      }
    }
    this.isDown = false;
  ...
  onMouseMoveHover(_e: MouseEvent) {
  }
  ...
  update() {
    if (this.clickPauseTimer > 0) {
      this.clickPauseTimer--;
      if (this.clickPauseTimer === 0) {
        this.clickedMediaIndex = -1;
      }
    } else if (!this.isDown) {
  ...
  addEventListeners() {
    ...
    this.boundOnMouseMoveHover = this.onMouseMoveHover;
...
const CircularGallery = ({
  ...
  useEffect(() => {
    ...
    const computedColor = computedStyle.color || "#1F1F1F";
